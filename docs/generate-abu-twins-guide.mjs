...
const children = [
  ...
  new Table({
    ...
    rows: [
      ...
      ["Version", "1.17"],
      ["Status", "Updated: Opening stock uses the Abu Twins shop Excel as it is"],
    ].map(([k, v]) =>
      new TableRow({
        children: [
...
h1("5. Words we use"),
  ...
  table(
    ...
    [
      ...
      ["Opening stock sheet", "The Excel Abu Twins already uses: one file per shop, with PHONES, ACCESSORIES, SCREEN, and LAPTOPS. Upload it on Upload stock."],
  ...
  table(
    ...
    [
      ...
      ["Stock uploader", "The person who loads the system from a sheet", "Upload the Abu Twins opening stock Excel per shop, or the older step-by-step sheets. Nothing else. No selling, no money, no approving."],
  ...
  table(
    ...
    [
      ...
      ["Stock", "Phones & items, Upload stock, Phone IMEIs, Shop stock, Goods on the way", "What you sell, loading the shelf from Excel, each phone number, what is here, what is still coming."],
  ...
  ...feature({
    ...
    what: "Upload stock loads what is on the shelf from Excel. The easiest way is the Abu Twins opening stock workbook: one file for one shop, with tabs named PHONES, ACCESSORIES, SCREEN, and LAPTOPS. The same columns appear on every tab: PRODUCT NAME, BRAND, CATEGORY, QTY/IMEI/SERIAL NO, CONDITION, SPECIFICATION, UNIT COST PRICE, and MIN.SELLING PRICE. Pick the shop, upload the file, and the system adds missing item names, books phones and laptops as In shop, and sets piece counts for cords and screens. Older four-step sheets remain lower on the page for later top-ups.",
    why: "The shop already counts stock in that Excel. Asking them to retype everything into a new Techvaults layout wastes a day and creates mistakes. The software should follow the shop's paper, not the other way around.",
    story: "Iwo Road finishes tonight's count on the opening stock Excel. The uploader opens Upload stock, picks Iwo Road, Ibadan, and uploads that file. Phones with IMEIs appear In shop. Cords show the piece count. Challenge sends its own file the same way. Sell now can start tomorrow on real shelf units.",
    steps: [
      "Sign in as uploader@abutwins.com or Super Admin.",
      "Open Upload stock from the left menu, under Stock.",
      "Under Abu Twins opening stock sheet, pick the shop this file belongs to.",
      "Upload the shop's opening stock Excel with PHONES, ACCESSORIES, SCREEN, and LAPTOPS filled.",
      "Open Shop stock and Phone IMEIs. Confirm the numbers match the sheet.",
      "If a line is wrong, fix the Excel and upload again. Nothing is half loaded.",
    ],
    expect: [
      "The opening stock card is at the top of Upload stock.",
      "A good file adds item names, phones In shop, and piece lines for that shop.",
      "A file with a bad line loads nothing and names the tab and line to fix.",
      "Sending the same phone twice leaves the first booking alone.",
      "After a good upload, Sell now can find those In shop IMEIs.",
      "The older four steps are still there for later top-ups.",
    ],
  }),
