...
const children = [
  ...
  new Table({
    ...
    rows: [
      ...
      ["Version", "1.16"],
      ["Status", "Updated: Loading the shops from a sheet, the stock uploader job, and receipts"],
    ].map(([k, v]) =>
      new TableRow({
        children: [
...
h1("5. Words we use"),
  ...
  table(
    ...
    [
      ...
      ["Stock uploader", "The person who loads the system from a sheet", "Upload the item list, the shelf counts, the phones by IMEI, and customers. Nothing else. No selling, no money, no approving."],
  ...
  ...feature({
    title: "10.39 Upload stock. Loading the shops from a sheet",
    what: "Upload stock is one screen with four steps, in the order the work has to happen. Step 1 is The item list. Step 2 is Pieces on the shelf. Step 3 is Phones by IMEI. Step 4 is Customers. Excel or CSV both work. Steps 2, 3, and 4 stay shut until step 1 is done.",
    why: "A shop cannot be put on a computer one line at a time. A container of two hundred phones typed by hand is an evening lost and a night of mistakes. The order is not a suggestion either: nothing can be counted or booked in until the system has been told what the item is. That is why the later steps stay shut until the item list is in.",
    story: "Abu Twins is putting Bodija on the system. The uploader sends the item list first: every model, one row each. The screen then opens the other three steps. He sends the shelf counts for cords and chargers, then the container sheet of two hundred and forty IMEIs. Bodija now shows two hundred and forty phones In shop, and Shop stock agrees with the IMEI list, because booking the phones in raised the shelf count at the same time.",
    steps: [
      "Sign in as uploader@abutwins.com.",
      "Open Upload stock from the left menu, under Stock.",
      "Read the column names listed under step 1, then prepare your sheet with those names in the first row.",
      "Upload the item list. Wait for the count at the top right of the step to rise.",
      "Upload Pieces on the shelf, then Phones by IMEI, then Customers.",
      "Deliberately put one wrong line in a sheet, such as a shop that does not exist, and upload it.",
      "Send the same phones sheet a second time.",
    ],
    expect: [
      "Steps 2, 3, and 4 are shut with a note until the item list is loaded.",
      "Each step shows how far the shop has got, such as how many items are on the list and how many phones are In shop.",
      "A sheet with a bad line loads nothing at all, and names the lines to fix by their spreadsheet line number.",
      "Sending the same sheet twice adds nothing the second time. A phone already on the system is left exactly as it is.",
      "After the phones sheet, Shop stock and Phone IMEIs agree, with no stock count needed.",
      "Column names are forgiving. Item code, SKU, and code all work. So do IMEI, IMEI1, and phone.",
      "Shops can be named in full or by their short code: IWO, BOD, CHL.",
    ],
  }),
