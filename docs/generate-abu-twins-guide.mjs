...
const children = [
  ...
  new Table({
    ...
    rows: [
      ...
      ["Version", "1.18"],
      ["Status", "Updated: Upload stock adds one item at a time or loads many from Excel"],
    ].map(([k, v]) =>
      new TableRow({
        children: [
...
h1("5. Words we use"),
  ...
  table(
    ...
    [
      ...
      ["Opening stock sheet", "The Excel Abu Twins already uses: one file per shop, with PHONES, ACCESSORIES, SCREEN, and LAPTOPS. Upload it on Upload stock when you have many lines."],
      ["Add one item to the shelf", "On Upload stock, book a single phone, laptop, or piece count by hand. Scan or type the IMEI or serial. The form clears for the next unit."],
  ...
  table(
    ...
    [
      ...
      ["Stock uploader", "The person who puts stock on the shelf in the system", "Add one phone or cord at a time on Upload stock, or upload the Abu Twins opening stock Excel per shop. Nothing else. No selling, no money, no approving."],
  ...
  table(
    ...
    [
      ...
      ["Stock", "Phones & items, Upload stock, Phone IMEIs, Shop stock, Goods on the way", "What you sell, adding stock one unit at a time or from Excel, each phone number, what is here, what is still coming."],
  ...
  ...feature({
    title: "10.39 Upload stock. One item or many from Excel",
    what: "Upload stock puts what is on the shelf into the system two ways. At the top, Add one item to the shelf lets staff pick the shop, pick or add the item name, and scan or type each IMEI, serial, or piece count one after another. When the form saves, the number fields clear for the next unit. Below that, the Abu Twins opening stock Excel still loads a full shop at once: one file per shop, with tabs named PHONES, ACCESSORIES, SCREEN, and LAPTOPS. Older four-step sheet uploads sit under Advanced for later top-ups.",
    why: "Not every delivery is a full carton count. Sometimes one iPhone arrives, or three cords. Forcing a spreadsheet for that feels heavy. Keeping the big Excel for opening stock and day-end counts still saves hours when hundreds of lines must go in at once.",
    story: "Challenge receives one iPhone 17 Pro Max before the rest of the carton. The uploader opens Upload stock, picks Challenge, Ibadan, chooses the model from the list, scans the IMEI, and clicks Add this unit to the shelf. The IMEI field clears. They scan the next phone the same way. That evening, Bodija sends its full opening stock Excel. The uploader picks Bodija and uploads the file. Phones appear In shop. Cords show the piece count. Sell now works on real shelf units the next morning.",
    steps: [
      "Sign in as uploader@abutwins.com or Super Admin.",
      "Open Upload stock from the left menu, under Stock.",
      "To add one unit, use Add one item to the shelf at the top.",
      "Pick the shop, pick the item from the list or add a new name with prices, then scan or type the IMEI, serial, or piece count.",
      "Click Add this unit to the shelf or Add pieces to the shelf. Confirm the fields clear for the next entry.",
      "For a full shop load, scroll to Abu Twins opening stock Excel, pick the shop, and upload the PHONES / ACCESSORIES / SCREEN / LAPTOPS file.",
      "Open Shop stock and Phone IMEIs. Confirm the numbers match what you entered.",
    ],
    expect: [
      "Add one item to the shelf is at the top of Upload stock.",
      "A phone with an IMEI books In shop and raises the shelf count by one.",
      "A cord with no number raises the piece count by the number you typed.",
      "Sending the same IMEI twice is refused or left alone if it is already on the system.",
      "A good opening stock file adds item names, phones In shop, and piece lines for that shop.",
      "A file with a bad line loads nothing and names the tab and line to fix.",
      "Advanced sheet uploads are folded away until you open them.",
    ],
  }),

  ...feature({
    title: "10.40 Stock uploader. A job that only loads data",
    what: "A job whose whole purpose is putting data into the system. It sees Upload stock, the item list, the phone list, and shop stock. It cannot sell, cannot open money pages, cannot approve anything, and cannot add staff.",
    why: "Loading the shops is powerful work and it is usually done by whoever is helping to set the system up, sometimes from outside the shop. That person needs no ability to sell or to see what the business is worth. Giving them one narrow job means the door they hold opens one room.",
    story: "Techvaults is helping Abu Twins load the three shops. They are given the Stock uploader login. They load the item list, the shelf counts, and every IMEI in all three shops. At no point can they open Money in and out, ring up a sale, or see what a customer owes.",
    steps: [
  ...
  table(
    ...
    [
      ...
      ["39", "Load opening stock from the Abu Twins Excel", "uploader@abutwins.com or Super Admin", "Pick the shop. Upload the PHONES / ACCESSORIES / SCREEN / LAPTOPS file. Phones show In shop. Piece counts appear on Shop stock. A bad line loads nothing."],
      ["40", "Add one item to the shelf by hand", "uploader@abutwins.com or Super Admin", "Pick the shop and item. Scan or type one IMEI or serial. Form clears for the next unit. Sell now finds the new In shop IMEI."],
    ],
    [600, 2800, 2600, 3360]
  ),
