...
const children = [
  ...
  new Table({
    ...
    rows: [
      ...
      ["Version", "1.29"],
      ["Status", "Ready for live shop use: reports by day, week, and month, larger stock uploads, till keeps working when the line drops"],
    ].map(([k, v]) =>
      new TableRow({
        children: [
...
h1("5. Words we use"),
  ...
  ...feature({
    ...
    what: "Settings holds the shop name, phone, address, and email that print on invoices and on Check the books. It also holds the low stock alert, default warranty days (starts at zero so the cashier sets days on the sale), and whether cashiers may sell below the lowest price. Only Super Admin can change these. Super Admin can also download a shop backup: a copy of shops, staff emails (not passwords), stock, IMEIs, sales, and purchases. Keep that file off this computer.",
