...
const children = [
  ...
  table(
    ...
    [
      ...
      ["Version", "1.1"],
      ["Status", "Updated: returns for sales reps, list sell price floor, till search and phone details"],
    ].map(([k, v]) => [k, v]),
    [2400, 4200]
  ),
  para("", { after: 200 }),
  ...
  h2("Sell now"),
  body("Scan an IMEI, or type to find by IMEI, phone name, brand, category, storage, or accessory. The line shows storage, Uk or Brand new, and colour under the name. You cannot sell under the list sell price unless Super Admin overrides. Selling above the list price is allowed, and Amount received updates with the new total so the day books match."),
  h2("Returns"),
