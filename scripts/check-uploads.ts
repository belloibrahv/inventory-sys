...
check("same IMEI twice counted once", Boolean(r.notices?.some((n) => n.includes("counted once"))))
check("duplicate IMEI is not a hard refuse", !r.problems.some((p) => p.includes("twice")))
check("first copy of duplicate IMEI is kept", r.rows.some((row) => row.imei1 === "860000000000304"))
