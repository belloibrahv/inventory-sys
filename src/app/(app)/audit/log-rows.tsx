...
export function AuditLogRows({ logs, resetKey = "" }: { logs: Log[]; resetKey?: string }) {
  const [open, setOpen] = useState<string | null>(null)
  const pager = usePagedRows(logs, resetKey || logs.length)
          ...
          return (
            ...
            >
              <div className="grid gap-2 md:grid-cols-[160px_1fr_auto] md:items-center">
                <p className="text-xs font-medium tabular-nums text-muted-foreground">{formatShopWhen(log.when)}</p>
                <div>
