...
> = [
  ...
  },
  {
    ...
    doThis: [
      ...
      "You can upload many items from Excel or CSV if that button is on your page.",
    ],
    watch: [
    ...
    lookup: ["product", "price", "sku", "item code", "upload"],
  },
  {
    id: "imei",
    href: "/imei",
  ...
  },
  {
    ...
    cannot: ["This page does not move a phone to another shop. Use Send to another shop."],
  ...
  },
  {
    ...
    what: "Book a carton before it reaches Ibadan. Phones go in as IMEIs. Cords go in as a piece count. The list stays Coming until someone says they have arrived.",
    doThis: [
  ...
  },
  {
    ...
    needAction: "action.intake",
    what: "A purchase from a supplier. It can sit as a bill, a part payment, or a carton that is still coming.",
    doThis: [
      "Pick the supplier and the shop.",
      "Add the items and the cost.",
      "Record what you paid. What you still owe stays on the bill.",
      "If the goods are still travelling, keep them Coming until they arrive.",
    ],
    watch: ["Buying and paying must leave a money trail."],
    cannot: ["This page does not invent a supplier. Add the supplier first."],
    lookup: ["purchase", "supplier bill", "paid"],
  },
  {
    id: "customers",
    href: "/customers",
  ...
  },
  {
    ...
    href: "/suppliers",
    title: "Suppliers",
    what: "People and firms Abu Twins buys from. Purchases and pay-outs hang on this list.",
    doThis: [
      "Add a supplier with a name and a phone.",
      "Open one to see unpaid bills.",
    ],
    watch: ["One supplier list keeps pay-outs honest."],
    cannot: ["This page does not receive a carton. Use Goods on the way or Goods from supplier."],
    lookup: ["supplier", "creditor"],
  },
  {
    id: "transfers",
    href: "/transfers",
    title: "Send to another shop",
    needAction: "action.transfer",
    what: "Move phones or accessories from one Ibadan shop to the other. The receiving shop must confirm arrival.",
    doThis: [
      "Pick the shop you are sending from and the shop that will receive.",
      "Scan the IMEIs that are leaving, or type an accessory quantity.",
      "Send. The phones are on the way, not sold, and not on the old shelf.",
      "At the other shop, confirm the IMEIs. Both shop stocks then update.",
    ],
    watch: ["If the other shop does not confirm, the system still knows the phones are on the road."],
    cannot: ["You cannot treat a transfer as a sale."],
    lookup: ["transfer", "send", "challenge", "iwo road"],
  },
  {
    id: "returns",
    href: "/returns",
    ...
    what: "A buyer brings a phone back. The original invoice stays. A new return record is made.",
    doThis: [
      "Find the sale. The buyer must have a name.",
      "Enter the IMEI and the reason.",
      "If a manager must say yes, wait on Needs approval.",
      "The phone comes back to the shop or goes to repair.",
    ],
  ...
  },
  {
    ...
    what: "Count what is physically here and compare it with the system. A manager must say yes before stock numbers change.",
    doThis: [
      "Count the boxes and IMEIs in front of you.",
      "Enter what you counted.",
      "Wait for approval. Stock does not move until yes.",
    ],
    watch: ["If IMEI vs shop count disagrees, this is the honest fix. Do not type a new number on Shop stock."],
    cannot: ["A no leaves the count rejected. Numbers stay as they were."],
    lookup: ["count", "recon", "mismatch"],
  },
  {
    id: "finance",
    href: "/finance",
  ...
  },
  {
    ...
    title: "Settings",
    needAction: "action.settings",
    what: "Shop name, phone, address, and email that print on invoices, books, reports, and this manual. Also the low stock alert and whether cashiers may sell below the lowest price.",
    doThis: [
      "Read the invoice header values.",
      "Change them only if Super Admin and the CEO agree.",
    ],
    watch: ["The next printed invoice and the next books PDF use these values."],
    cannot: ["CEO can read settings but cannot change them."],
    lookup: ["settings", "address", "phone", "lowest price"],
  },
  {
    id: "account",
    href: "/account",
...
function alwaysSections(keys: Set<string>): ManualSection[] {
  const extras: ManualSection[] = [
    {
      id: "search",
      title: "Find IMEI, invoice, or customer",
      group: "On every screen",
      what: "The search box on the top bar, or the search window from the keyboard. Type a phone number, an invoice number, a supplier bill, or a buyer name. The system jumps you to that record.",
      doThis: [
  ...
  if (keys.has("action.sell") && keys.has("view.pos")) {
    extras.push({
      ...
      group: "On every screen",
      what: "If Sell now has been opened on this device today and the till is not locked, you can still finish a sale. It stays on this device as a parked sale. Refresh is safe. When the line returns, it posts, or you tap Send parked work now.",
      doThis: [
        "Open Sell now while the line is up at least once that day, so this phone can keep the till.",
        "If the line drops, finish the sale anyway. You should see that it is saved on this device.",
        "If a refresh lands on The till is still here, parked sales are listed. Open Sell now when the cache has it, or send parked work when the line returns.",
        "When the yellow banner says the line is back, send parked work.",
      ],
      watch: [
        "A parked sale lives in a store on this phone, not only in a small browser notepad.",
        "A parked sale that sits more than two hours alerts Super Admin, the CEO, and the records checker.",
      ],
      cannot: ["You cannot park a new live sale while an older day with sales is still open."],
      lookup: ["offline", "line down", "parked", "banner", "refresh", "till is still here"],
    })
  }
